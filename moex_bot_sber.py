...
class MoexSignalBot:
    def __init__(self, telegram_token, chat_id):
        ...
        self.ticker = 'SBER'  # Временно используем Сбербанк
    ...
    async def get_stock_data(self, days=10):
    ...
    def calculate_signals(self, df):
        ...
        if current_close > buy_threshold:
            logger.info(f"🎯 Сформирован сигнал BUY: {current_close:.2f} > {buy_threshold:.2f}")
            signals.append({
                'type': 'BUY',
                'price': current_close,
        ...
        if current_close < sell_threshold:
            logger.info(f"🎯 Сформирован сигнал SELL: {current_close:.2f} < {sell_threshold:.2f}")
            signals.append({
                'type': 'SELL', 
                'price': current_close,
    ...
    def format_status_message(self, df):
        ...
        return (
            ...
            f"🔼 BUY порог: {buy_threshold:.2f} руб.\n"
            f"🔽 SELL порог: {sell_threshold:.2f} руб.\n"
...
async def main():
    try:
        ...
        await bot.send_message(f"🤖 Бот сигналов для {bot.ticker} запущен! (aiomoex)")
        ...
        logger.info("⏰ Бот запущен. Следующая проверка в 19:00")
